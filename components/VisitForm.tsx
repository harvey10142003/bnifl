  ...
  if (status === 'success') {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="font-serif text-2xl text-ink-900 mb-3">預約已送出</h3>
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      <div className="grid gap-5 md:grid-cols-2">
        <Field label="姓名" name="name" required placeholder="王小明" />
        <Field label="聯絡電話" name="phone" type="tel" required placeholder="0912-345-678" />
      </div>
      <Field label="Email" name="email" type="email" required placeholder="you@company.com" />
      <Field label="公司名稱" name="company" required placeholder="○○有限公司" />
      <Field label="職稱" name="title" required placeholder="總經理 / 業務經理" />
      <div>
        <label className="text-sm font-medium text-ink-900 block mb-2">
          產業類別 <span className="text-platinum-700">*</span>
        </label>
        <select
          name="industry"
          required
      ...
      <div>
        <label className="text-sm font-medium text-ink-900 block mb-2">希望參訪日期</label>
        <input
          type="date"
          name="visitDate"
          className="w-full bg-pearl border border-ink-200 px-4 py-3 text-ink-900 focus:border-platinum-500 focus:outline-none transition-colors"
      ...
      <div>
        <label className="text-sm font-medium text-ink-900 block mb-2">推薦人（如有）</label>
        <input
          type="text"
          name="referrer"
          placeholder="會員姓名"
      ...
      <div>
        <label className="text-sm font-medium text-ink-900 block mb-2">想多認識什麼產業？</label>
        <textarea
          name="message"
          rows={3}
...
function Field({
  ...
  return (
    <div>
      <label className="text-sm font-medium text-ink-900 block mb-2">
        {label}
        {required && <span className="text-platinum-700 ml-1">*</span>}
      </label>
      <input
        type={type}
        name={name}
